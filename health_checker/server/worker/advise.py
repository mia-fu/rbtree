...
class Advise(object):
    ...
    binlog_size_too_large = "您的磁盘空间为{0}，您的binlog占用空间为{1}，" \
                            "超过了阈值{2}%，建议您适当调小expire_logs_days，减少二进制日志保留时间"
    ...
    innodb_log_file_size = "您的磁盘空间为{0}，您的数据库配置参数innodb_log_file_size取值为{1}，" \
                           "该值太大会影响数据库故障恢复的速度，该值太小会影响数据库的性能，根据您的磁盘空间，我们建议您取值为{2}到{3}之间"

    max_connection_warning = "数据库连接数过多容易造成线程频繁的上下文切换，连接数过少不能充分发挥数据库的性能，您的数据库连接数是{0}，" \
                             "你的buffer pool大小是{1}，建议您的数据库连接数设置在{2}~{3}范围内，也可以直接设置为{4}"
...
# RedundancyPrefixIndex=根据索引最左前缀匹配原则，
# 您的数据库{0}下的表{1}中{2}索引是多余的，因为您已经创建了{3}索引
#
...
# BufferTooLargeInTheory=您的系统可用内存为{0}，
# 您的配置参数max_connections为{1}，理论上您的数据库最大占用内存为{2}，建议调整Innodb_buffer_pool_size, max_connections参数或者升级规格
#
# # 容量规划优化建议
