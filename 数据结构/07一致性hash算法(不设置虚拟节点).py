...
class conhashnorep(object):
    def __init__(self, nodes=None):
        """此处nodes代表需要初始化的node列表，为进行对比先提供虚拟节点副本数,
        ring是存储hash-node的字典，key是存储所有hash值的列表"""
        self.nodes = nodes
        self.ring = {}
        self._sorted_keys = []
    ...
    def remove_nodes(self, nodes):
        """对于remove的操作，如果直接操作已经产生的ring字典，
        会比较麻烦，因为ring是{hashnode：node}的键值对形式，
        直接处理涉及到字典直接查找值ring.values()/ring.keys()，
        以及通过值来remove(key)的操作，太麻烦"""
        if nodes:
            for node in nodes:
                bnode = node
                nodehash = abs(crc32(bnode))
